...
Panel {
  ...
  // Click-to-expand state for the forecast row: the date currently showing
  // its hourly breakdown, or "" when no day is expanded.
  property string selectedForecastDate: ""
  ...
  readonly property var hourlyRows: Model.hourlyForecastForDate(dailyForecastReport, selectedForecastDate)
  ...
  function dayName(dateString) {
    if (String(dateString).slice(0, 10) === Qt.formatDate(new Date(), "yyyy-MM-dd")) return "Today"
    return Model.dayName(dateString, function(date) { return Qt.formatDate(date, "dddd") })
  }

  // Expand/collapse the hourly breakdown for a forecast tile; clicking the
  // already-expanded day collapses it.
  function toggleForecastDay(date) {
    root.selectedForecastDate = (root.selectedForecastDate === date) ? "" : date
  }

  function hourLabel(hourString) {
    return Model.hourLabel(hourString)
  }

  function bareHourlyTemp(hourRow) {
    if (!hourRow) return ""
    var v = useImperial ? hourRow.tempF : hourRow.tempC
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
        ...
        Row {
          ...
          Repeater {
            ...
            Rectangle {
              ...
              required property var modelData
              required property int index
              readonly property bool selected: root.selectedForecastDate === modelData.date
              width: dayCellRow.implicitWidth + Style.space(16)
              height: dayCellRow.implicitHeight + Style.space(10)
              radius: Style.cornerRadius
              color: selected || dayCellHover.hovered ? Style.hoverFillFor(root.bar.foreground, Color.accent) : "transparent"
              ...
              TapHandler {
                onTapped: root.toggleForecastDay(dayCell.modelData.date)
              }
              ...
              Row {
                ...
                Text {
                  textFormat: Text.PlainText
                  anchors.verticalCenter: parent.verticalCenter
                  text: root.dayIcon(dayCell.modelData)
                  color: root.bar.foreground
                  font.family: root.bar.fontFamily
                  font.pixelSize: Style.font.display
                }
                ...
                Column {
                  ...
                  Text {
                    textFormat: Text.PlainText
                    text: root.dayName(dayCell.modelData.date).toUpperCase()
                    color: dayCell.selected ? Style.hoverStateColor(root.bar.foreground, Color.accent) : Qt.darker(root.bar.foreground, 1.4)
                    font.family: root.bar.fontFamily
                  ...
                  Row {
                    ...
                    Text {
                      textFormat: Text.PlainText
                      text: root.bareTempForDay(dayCell.modelData, "max")
                      color: root.bar.foreground
                      font.family: root.bar.fontFamily
                      font.pixelSize: Style.font.body
                    }
                    Text {
                      textFormat: Text.PlainText
                      text: root.bareTempForDay(dayCell.modelData, "min")
                      color: Qt.darker(root.bar.foreground, 1.5)
                      font.family: root.bar.fontFamily
                      font.pixelSize: Style.font.body
                    }
      ...
      // ---- Hourly breakdown for the tapped forecast day (today included).
      Column {
        visible: root.selectedForecastDate !== "" && root.hourlyRows.length > 0
        width: parent.width
        spacing: Style.space(10)
        ...
        Text {
          textFormat: Text.PlainText
          text: root.dayName(root.selectedForecastDate).toUpperCase() + " · HOURLY"
          color: Qt.darker(root.bar.foreground, 1.4)
          font.family: root.bar.fontFamily
          font.pixelSize: Style.font.caption
          font.letterSpacing: 1
        ...
        Flickable {
          ...
          contentWidth: hourlyRow.implicitWidth + Style.space(32)
          clip: true
          boundsBehavior: Flickable.StopAtBounds
          ...
          Row {
            ...
            Repeater {
              ...
              Column {
                ...
                Text {
                  textFormat: Text.PlainText
                  text: root.hourLabel(modelData.hour)
                  color: Qt.darker(root.bar.foreground, 1.4)
                  font.family: root.bar.fontFamily
                  font.pixelSize: Style.font.caption
                  anchors.horizontalCenter: parent.horizontalCenter
                ...
                Text {
                  textFormat: Text.PlainText
                  text: root.iconForOpenMeteoCode(modelData.openMeteoWeatherCode, Number(modelData.isDay) === 0)
                  color: root.bar.foreground
                  font.family: root.bar.fontFamily
                  font.pixelSize: Style.font.title
                  anchors.horizontalCenter: parent.horizontalCenter
                ...
                Text {
                  textFormat: Text.PlainText
                  text: root.bareHourlyTemp(modelData)
                  color: root.bar.foreground
                  font.family: root.bar.fontFamily
                  font.pixelSize: Style.font.body
                  anchors.horizontalCenter: parent.horizontalCenter
