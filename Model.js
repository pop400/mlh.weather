...
// Includes today as the first entry, so a tile for the current day is always
// clickable alongside the upcoming ones.
...
// wttr.in's "weather" array starts with today; kept in step with
// openMeteoForecastDays above, which also includes today.
...
// Hourly rows for one YYYY-MM-DD date, drawn from Open-Meteo's hourly block
// (requested alongside the daily forecast). Works for today too, since
// Open-Meteo's hourly range always starts at the beginning of the current day.
function hourlyForecastForDate(dailyForecastReport, dateString) {
  var hourly = dailyForecastReport && dailyForecastReport.hourly ? dailyForecastReport.hourly : null
  if (!hourly || !hourly.time || !dateString) return []

  var result = []
  for (var i = 0; i < hourly.time.length; ++i) {
    var time = String(hourly.time[i] || "")
    if (time.slice(0, 10) !== dateString) continue
    ...
    result.push({
      ...
      isDay: hourly.is_day ? hourly.is_day[i] : null
    })
  }
  return result
}

// "3 PM" style label from a 24-hour "HH" string.
function hourLabel(hourString) {
  var h = parseInt(String(hourString || ""), 10)
  if (isNaN(h)) return ""
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    hourlyForecastForDate: hourlyForecastForDate,
    hourLabel: hourLabel,
    dayIcon: dayIcon,
